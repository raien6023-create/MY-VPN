...
def handle_all_messages(message):
    ...
    if message.content_type == 'photo':
        ...
        admin_msg = f"💰 **فیش واریزی جدید واصل شد!**\n\n👤 فرستنده: {message.from_user.first_name}\n🆔 آیدی عددی: `{user_id}`\nیوزرنیم: @{message.from_user.username or 'ندارد'}\n\n👉 جهت ارسال کانفیگ به این کاربر، روی همین پیام عکس ریپلای کنید."
        bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=admin_msg)
        return
